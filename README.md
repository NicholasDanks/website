...
- **A site key on Google's free tier, direct to Google.** The Gemini REST API (`streamGenerateContent` with function calling) is called from the browser; no SDK is bundled and there is no server. The page carries a site-owned free-tier key, injected at build time from the `PUBLIC_GEMINI_API_KEY` environment variable (set in Netlify; never committed), so visitors pay nothing. Google's Gemini keys are bound to a service account and cannot be referrer-restricted, so the key is extractable from the page; the project must never carry billing, and the key is rotated in AI Studio if usage looks abnormal; a visitor can paste their own key instead (kept in `sessionStorage`, or `localStorage` on opt-in). The page discloses that the free tier lets Google use what is sent to improve its products.
